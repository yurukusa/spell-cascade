...
タイトル画面に「← Yesterday MM/DD」ボタンを追加。前日のデイリーチャレンジシードで遊べる。

**設計意図**: 日本時間22時ごろ遊び始めて日付をまたいだプレイヤーが「昨日のやつに参加できなかった」問題を解消。
24時間の猶予を設けることで、itch.io コメント欄での比較議論が翌日も続く。

- タイトル画面に控えめなオレンジ（やや暗め）のサブボタンとして表示
- シード計算は今日と同じ式を昨日の日付で適用 (`unix_time - 86400`)
- リザルト画面バッジ: 「★ PAST CHALLENGE MM/DD ★」（今日とは区別）
- シェアテキスト: `[Spell Cascade Past MM/DD]`（コメント欄での日付識別に使用）
